...
class MetricsCollector:
    ...
    def calcular_precision(self, respuesta: str) -> float:
        ...
        """
        if "No encontré información" in respuesta or "No tengo suficiente información" in respuesta:
            return 0.0
    ...
    def generar_resumen(self) -> str:
        ...
        precision_promedio = sum(self.metrics["precision_respuestas"]) / len(self.metrics["precision_respuestas"]) if self.metrics["precision_respuestas"] else 0
...
def test_optimized_rag():
    ...
    try:
        ...
        # Preguntas de prueba
        test_questions = [
            "¿Cuáles son los principales temas tratados en los documentos?",
            "¿Explicame integrales dobles?",
            "¿Explicame integrales triples?"
        ]
